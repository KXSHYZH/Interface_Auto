...
class PATH:
    ...
    config_tmp_path = os.path.dirname(os.path.realpath(__file__)) + '/../Template/template'  # 添加代码
...
class ReportTestResult(unittest.TestResult):
    ...
    def stopTestRun(self, title=None) -> dict:
        ...
        FIELDS['end_time'] = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())  # 添加代码
        start_time = int(time.mktime(time.strptime(self.begin_time, '%Y-%m-%d %H:%M:%S')))
        end_time = int(time.mktime(time.strptime(FIELDS['end_time'], '%Y-%m-%d %H:%M:%S')))  # 添加代码
    ...
    def add_test_type(self, status: str, case_log: list) -> None:
        """
            abstruct add Noneed type and return tuple
        :param status:
        :param case_log:
        :return:
        """
    ...
    def get_testcase_property(test) -> tuple:
        ...
        """
        # class_name = Noneed.__class__.__qualname__
        # method_name = Noneed.__dict__['_testMethodName']
        # method_doc = Noneed.__dict__['_testMethodDoc']
        # 以下为更改后
        class_name = test.table_header['_caseId']
        method_name = test.table_header['_testModule']
        method_doc = test.table_header['_testDescribe']
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def report(self, description, filename: str = None, report_path='.'):
        ...
        self.output_report()
        text = '\n测试已全部完成, 可前往"{}"查询测试报告'.format(self.report_path + '\\' + self.filename)
        print(text)
